    ...
    if opt.run_unlearn:
        ...
        if opt.mode == "HR":
            ...
            if opt.method == "DUCK" or opt.method == "RandomLabels":
                ...
                syn_dset = SyntDataset(load_synt=opt.load_synt, save_folder=os.path.join(opt.root_folder+"/syn_dst_"+opt.dataset),train_loader=train_loader,pretr_model=pretr_model, path=os.path.join(opt.root_folder+"/syn_dst_"+opt.dataset))
                train_fgt_syn,train_retain_syn=split_retain_forget(syn_dset, class_to_remove)
